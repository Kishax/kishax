package keyp.forev.fmc.neoforge.server.cmd.main;
...
import keyp.forev.fmc.neoforge.server.cmd.sub.ReloadConfig;
...
public class FMCCommand {
  ...
  public static int execute(CommandContext<CommandSourceStack> context, String subcommand) throws CommandSyntaxException {
    ...
    try {
      if (!Main.getInjector().getInstance(NeoForgeLuckperms.class).hasPermission(source, "fmc." + subcommand)) {
