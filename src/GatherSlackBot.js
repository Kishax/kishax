...
class GatherSlackBot {
	constructor() {
		...
		this.connectionManager = new ConnectionManager(this.gatherSpaceId, this.gatherApiKey);

		console.log("🔧 設定確認:");
		console.log(`- Space ID: ${this.gatherSpaceId}`);
		console.log(`- API Key: ${this.gatherApiKey ? "設定済み" : "未設定"}`);
		console.log(`- Slack Webhook: ${this.slackWebhookUrl ? "設定済み" : "未設定"}`);
	}

	async loadInitialUsers() {
		try {
			const currentMembers = await this.userManager.loadInitialUsers(this.connectionManager.getGame());
			...
			// 定期的な接続監視を開始
			this.connectionManager.startConnectionMonitoring(this.userManager, this.slackNotifier);
		} catch (error) {
	...
	async processPendingEvents() {
		if (this.pendingEvents.length > 0) {
			console.log(`🔄 初期ロード完了後の保留イベント処理開始: ${this.pendingEvents.length}件`);

			for (const event of this.pendingEvents) {
				console.log(`🔄 保留イベント処理: ${event.type} (ID: ${event.playerId})`);

				if (event.type === "playerJoins") {
					if (!this.userManager.hasUser(event.playerId)) {
						console.log(`✅ 保留イベントを新規参加として処理: ${event.playerId}`);
						await this.handlePlayerJoins(event.data, event.context);
					} else {
						console.log(`! 保留イベントは初期メンバーのため無視: ${event.playerId}`);
					}
	...
	async handlePlayerJoins(data, context) {
		...
		if (!this.userManager.hasUser(playerId)) {
			...
			// ユーザー名取得
			const result = await this.userManager.getUserName(playerId, this.connectionManager.getGame(), data, context);
			this.userManager.addUser(playerId, result.name);

			console.log(`👤 ユーザー参加確定: ${result.name} (遅延取得: ${result.isDelayed})`);

			// Slack通知を送信
			await this.slackNotifier.notifyUserJoined(result.name);
		} else {
			console.log(`! 既に接続済みのユーザーのため通知スキップ: ${playerId}`);
			console.log(`📊 キャッシュされた名前: ${this.userManager.getCachedUserName(playerId)}`);
		}
	}
	...
	async handlePlayerLeaves(data, context) {
		...
		console.log(`📊 connectedUsersに含まれている？: ${this.userManager.hasUser(playerId)}`);

		if (this.userManager.hasUser(playerId)) {
			console.log(`✅ 退出処理開始: ${playerId}`);
			
			console.log("📊 完全な退出イベントデータ:", JSON.stringify(data, null, 2));
			console.log("📊 完全な退出contextデータ:", JSON.stringify(context, null, 2));

			// 退出時は遅延取得なし（キャッシュまたは即座に取得可能な情報のみ）
			const result = await this.userManager.getUserName(playerId, this.connectionManager.getGame(), data, context, true);
			
			this.userManager.removeUser(playerId);

			console.log(`👤 ユーザー退出確定: ${result.name}`);
			console.log(`📊 退出後の接続ユーザー: [${this.userManager.getAllUserIds().join(", ")}]`);
			...
			console.log(`📊 現在の接続ユーザー一覧: [${this.userManager.getAllUserIds().join(", ")}]`);
		}
	}
	...
	async connect() {
		try {
			...
			// プレイヤー参加イベント
			this.connectionManager.subscribeToEvent("playerJoins", async (data, context) => {
				try {
					const playerId = context.playerId;

					console.log(`📥 プレイヤー参加イベント受信 (ID: ${playerId})`);
					console.log(`📊 初期ロード完了: ${this.initialUsersLoaded}`);
					console.log(`📊 現在の接続ユーザー: [${this.userManager.getAllUserIds().join(", ")}]`);
					console.log(`📊 このユーザーは既に接続済み？: ${this.userManager.hasUser(playerId)}`);
			...
			// プレイヤー退出イベント
			this.connectionManager.subscribeToEvent("playerLeaves", async (data, context) => {
				try {
					const playerId = context.playerId;

					console.log(`📤 プレイヤー退出イベント受信 (ID: ${playerId})`);
					console.log(`📊 現在の接続ユーザー: [${this.userManager.getAllUserIds().join(", ")}]`);
					console.log(`📊 このユーザーは接続済み？: ${this.userManager.hasUser(playerId)}`);
			...
			// プレイヤー移動イベント（デバッグ用）
			this.connectionManager.subscribeToEvent("playerMoves", (data, context) => {
				// console.log(`🚶 ${context.playerId} が移動しました`);
			});

			// チャットメッセージイベント
			this.connectionManager.subscribeToEvent("playerChats", async (data, context) => {
				try {
					const playerId = context.playerId;
					const result = await this.userManager.getUserName(playerId, this.connectionManager.getGame(), null, context);
