...
public class DoServerOnline2 {
	...
	private final ConfigUtils cutil;
    ...
    @Inject
    public DoServerOnline2 (Main plugin, Config config, ConfigUtils cutil, ProxyServer server, Logger logger, Database db, ConsoleCommandSource console) {
    	this.plugin = plugin;
		this.config = config;
		this.cutil = cutil;
    	this.logger = logger;
						...
						if (rsAffected > 0) {
							...
							try (PreparedStatement ps3 = conn.prepareStatement(query3)) {
								try (ResultSet status = ps3.executeQuery()) {
									ResultSetMetaData metaData = status.getMetaData();
        							int columnCount = metaData.getColumnCount();
									List<String> statusColumnsList = new ArrayList<>();
									for (int i = 1; i <= columnCount; i++) {
										statusColumnsList.add(metaData.getColumnName(i));
									}
									while (status.next()) {
										...
										if (!velocityToml.containsKey(serverDBName)) {
											...
											// DBに存在せず、Tomlに存在するサーバー(追加・更新対象)
											
											// 以下、〇〇がTomlから得られるconfigとdatabaseで異なる場合、config優先
											// ポート番号
											int serverDBPort = status.getInt("port");
											...
											String serverDBConfigType = config.getString("Servers." + serverDBName + ".type", null),
													serverDBType = status.getString("type"),
													serverDBConfigPlatform = config.getString("Servers." + serverDBName + ".platform", null),
													serverDBPlatform = status.getString("platform");
											// ポート番号
											...
											try (PreparedStatement ps5 = conn.prepareStatement(query5)) {
												...
												if (!serverDBConfigPlatform.equalsIgnoreCase(serverDBPlatform)) {
													...
													console.sendMessage(Component.text(serverDBName+"サーバーのプラットフォ");
												}
											}
										}
									}
									velocityToml.entrySet().stream().forEach(entry -> {
