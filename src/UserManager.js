class UserManager {
	...
	async getUserName(playerId, game, data = null, context = null, isRetry = false) {
		if (this.userNameCache.has(playerId)) {
			const cachedName = this.userNameCache.get(playerId);
			console.log(`🔍 キャッシュからユーザー名取得: ${cachedName} (ID: ${playerId})`);
			...
			return new Promise((resolve) => {
				setTimeout(async () => {
					try {
						...
						if (delayedPlayer && delayedPlayer.name && delayedPlayer.name.trim() !== "") {
							...
							console.log(`! 遅延取得でも名前が見つかりません (ID: ${playerId})`);
		...
		const fallbackName = "新しいユーザー";
		console.log(`! ユーザー名取得失敗、デフォルト値を使用: ${fallbackName} (ID: ${playerId})`);
		this.userNameCache.set(playerId, fallbackName);
