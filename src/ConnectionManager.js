...
class ConnectionManager {
	...
	async checkConnectionStatus(userManager, slackNotifier) {
		try {
			...
			console.log(`📊 追跡中のユーザー: [${userManager.getAllUserIds().join(", ")}]`);
			...
			for (const user of changes.leftUsers) {
				...
				console.log(`👤 ユーザー退出確定（定期チェック）: ${user.name} (ID: ${user.id})`);
			...
			for (const user of changes.joinedUsers) {
				if (!userManager.isRecentJoinEvent(user.id, 60000)) {
					...
					console.log(`👤 ユーザー参加確定（定期チェック）: ${userName} (ID: ${user.id})`);
			...
			console.log(`✅ 定期チェック完了 - 現在の追跡ユーザー数: ${userManager.getUserCount()}`);
		} catch (error) {
