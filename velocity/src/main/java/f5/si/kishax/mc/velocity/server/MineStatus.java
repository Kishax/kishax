...
public class MineStatus {
  ...
  @Inject
  public MineStatus(Logger logger, Database db, Provider<SocketSwitch> sswProvider) {
    this.logger = logger;
    this.db = db;
            ...
            if (beforeServerName != null && currentServerName == null) {
              if (serverName.equals("proxy")) {
                ...
                try (PreparedStatement ps2 = conn.prepareStatement(query2)) {
                  ...
                  rsAffecteds2.add(rsAffected2);
                  if (rsAffected2 > 0) {
                    // logger.info("Player {} has been removed from server {}", playerName,
                    // "proxy");
                  }
                }
              }
            ...
            if (currentServerName != null) {
              if (serverName.equals(currentServerName)) {
                ...
                try (PreparedStatement ps2 = conn.prepareStatement(query2)) {
                  ...
                  rsAffecteds2.add(rsAffected2);
                  if (rsAffected2 > 0) {
                    // logger.info("Player {} has been added to server {}", playerName,
                    // currentServerName);
                  }
                }
              }
