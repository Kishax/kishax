  ...
  public void execute(@NotNull CommandSource source, String[] args) {
    ...
    try (Connection conn = db.getConnection(); 
      ...
      try (ResultSet ismente = ps.executeQuery();
        ...
        switch (args.length) {
          case 0, 1 -> source.sendMessage(Component.text("usage: /kishaxp maintenance <switch|status|list|add|remove> <player|discord> <true|false>").color(NamedTextColor.GREEN));
          case 2 -> {
            switch(args[1].toLowerCase()) {
              ...
              case "switch" -> source.sendMessage(Component.text("usage: /kishaxp maintenance switch discord <true|false>").color(NamedTextColor.GREEN));
              case "add", "remove" -> source.sendMessage(Component.text("usage: /kishaxp maintenance <add|remove> <player>").color(NamedTextColor.GREEN));
              case "list" -> {
              ...
              default -> source.sendMessage(Component.text("usage: /kishaxp maintenance <switch|status|list|add|remove> <player|discord> <true|false>").color(NamedTextColor.GREEN));
            }
          }
          case 3 -> {
            switch (args[1].toLowerCase()) {
              ...
              case "switch" -> {
                if(!(args1.contains(args[1].toLowerCase()))) {
                  source.sendMessage(Component.text("第2引数が不正です。").color(NamedTextColor.RED).append(Component.text("usage: /kishaxp maintenance <switch|status> <discord> <true|false>").color(NamedTextColor.GREEN)));
                  break;
                }
                if(!(args2.contains(args[2].toLowerCase()))) {
                  source.sendMessage(Component.text("第3引数が不正です。").color(NamedTextColor.RED).append(Component.text("usage: /kishaxp maintenance <switch|status> <discord> <true|false>").color(NamedTextColor.GREEN)));
                  break;
                }
                source.sendMessage(Component.text("discord通知をtrueにするかfalseにするかを決定してください。").color(NamedTextColor.RED).append(Component.text("usage: /kishaxp maintenance <switch|status> <discord> <true|false>").color(NamedTextColor.GREEN)));
              }
              default -> source.sendMessage(Component.text("usage: /kishaxp maintenance <switch|status|list|add|remove> <player|discord> <true|false>").color(NamedTextColor.GREEN));
            }
          }
            ...
            switch (args[1].toLowerCase()) {
              case "switch" -> {
                ...
                if (!(args3.contains(args[3].toLowerCase()))) {
                  source.sendMessage(Component.text("第4引数が不正です。").color(NamedTextColor.RED).append(Component.text("usage: /kishaxp maintenance <switch|status> <discord> <true|false>").color(NamedTextColor.GREEN)));
                  break;
                }

                if (!(args[3].equals("true") || args[3].equals("false"))) {
                  source.sendMessage(Component.text("trueかfalseを入力してください。").color(NamedTextColor.RED).append(Component.text("usage: /kishaxp maintenance <switch|status> <discord> <true|false>").color(NamedTextColor.GREEN)));
                  break;
                }
