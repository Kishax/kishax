package keyp.forev.fmc.velocity.socket.message.handlers.minecraft.server;
...
public class VelocityServerActionHandler implements ServerActionHandler {
    ...
    @Inject
    public VelocityServerActionHandler(Logger logger, ProxyServer server, Database db, BroadCast bc, ConsoleCommandSource console) {
        this.logger = logger;
        this.server = server;
        this.db = db;
        this.bc = bc;
        ...
        switch (mserver.action) {
            case "START" -> {
                TextComponent component = Component.text()
                        .append(Component.text(serverName + "サーバーが起動しました。").color(NamedTextColor.GOLD).decorate(TextDecoration.BOLD))
                        .build();
